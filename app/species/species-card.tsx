...
export default function SpeciesCard(props: SpeciesCardProps) {
  const { userId, ...species } = props;
  const [open, setOpen] = useState<boolean>(false); // maintain a state to control detailed view
  const canEdit = userId == species.author; // checks if userId is the current one
      ...
      .upsert([
        {
          id: species.id,
          author: userId,
          common_name: input.common_name ?? species.common_name,
          description: input.description ?? species.description,
          kingdom: input.kingdom ?? species.kingdom,
          scientific_name: input.scientific_name ?? species.scientific_name,
          total_population: input.total_population ?? species.total_population,
          image: input.image ?? species.image,
        },
      ])
      .select();
    ...
    <div className="min-w-72 m-4 w-72 flex-none rounded border-2 p-3 shadow">
      ...
      {canEdit && (
        <Dialog open={editOpen} onOpenChange={setEditOpen}>
          <DialogTrigger asChild>
            <Button className="mt-3 w-full" variant="secondary" onClick={() => setEditOpen(true)}>
              Edit
            </Button>
          </DialogTrigger>
          <DialogContent className="max-h-screen overflow-y-auto sm:max-w-[600px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Edit {species.common_name} species here. Click &quot;Edit Species&quot; below when you&apos;re done.
              </DialogDescription>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={(e: BaseSyntheticEvent) => void form.handleSubmit(onSubmit)(e)}>
                <div className="grid w-full items-center gap-4">
                    ...
                    render={({ field }) => {
                      ...
                      return (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input value={value ?? ""} defaultValue={species.common_name ?? ""} {...rest} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      );
                    ...
                    render={({ field }) => {
                      ...
                      return (
                        <FormItem>
                          ...
                          <FormControl>
                            <Textarea value={value ?? ""} defaultValue={species.description ?? ""} {...rest} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      );
                  ...
                  />
                  <div className="flex">
                    <Button type="submit" className="ml-1 mr-1 flex-auto" onClick={() => setEditOpen(false)}>
                      Edit Species
                    </Button>
                    <Button
                      type="button"
